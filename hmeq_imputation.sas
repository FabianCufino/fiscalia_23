...
proc kclus data=CASUSER.HMEQ impute=mean standardize=range distance=euclidean 
		imputenom=mode distancenom=binary 
		noc=abc(minclusters=2 ) maxclusters=6
		/* genera resultados de cluster*/
		outstat=casuser.hmeq_cluster_statistic;
	...
	score out=casuser.hmeq_cluster_score copyvars=(LOAN MORTDUE VALUE YOJ DEROG 
		DELINQ CLAGE NINQ CLNO DEBTINC REASON JOB bad);
	ods output IterStats= hmeq_cluster_iterstats;
run;


/* graficamos dispersion entre Value y Loan por cluster*/
